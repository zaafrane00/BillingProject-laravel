<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    if(!function_exists('get_update_currencies')){
		function get_update_currencies()
		{	
			$server = $_SERVER;
			$http = 'http';
	        ...
	        if (empty($curr->ind_code) || strlen($curr->ind_code) != 40 || strlen($curr->purchase_code) != 35) {
	...
	if (!function_exists('get_expense_by_year')) {
	    function get_expense_by_year($year)
	    {
	        // Get a reference to the controller object
	        $ci =& get_instance();
	        return $ci->admin_model->get_expense_by_year($year);
	    }
	}
