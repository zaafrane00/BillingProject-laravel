                  ...
                  <h4 style="margin-top: 0"><?php echo html_escape($type) ?> <?php echo html_escape($invoice->number) ?></h4>
                    <p style="color: #555; font-size: 18px; margin-bottom: 30px"><?php echo html_escape($type) ?> for 
                      <strong>
                        ...
                        <?php else: ?>
                          <?php echo html_escape($currency_symbol) ?><?php echo html_escape($invoice->grand_total) ?> <?php echo html_escape($currency_code) ?>
                        <?php endif; ?>
                      </strong>
                      due by <strong><?php echo my_date_show($invoice->created_at) ?></strong></p>

                      <?php if ($type=='Invoice'): ?><?php $link_type = 'invoice'; ?><?php else: ?><?php $link_type = 'estimate'; ?><?php endif ?>

                    <a target="_blank"  href="<?php echo base_url('readonly/'.$link_type.'/view/'.md5($invoice->id)) ?>" style="text-decoration: none;background: #1e88e5;padding: 15px 25px;border-radius: 50px;font-size: 16px;font-weight: 600;color: #fff;">View <?php echo html_escape($type) ?></a><br><br>
                  </td>
